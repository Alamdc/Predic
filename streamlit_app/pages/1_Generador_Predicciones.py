...
# Asegúrate de que la carpeta 'utils' tenga los archivos __init__.py, modeling.py y db_client.py
...
st.title("Generador de Predicciones de Flujo (XGBoost+)")

# ==========================================
# 1. BARRA LATERAL (FILTROS Y CONFIG)
# ==========================================
with st.sidebar:
    st.header("1. Filtros de Datos (API)")
    ...
    start = col_d1.date_input("Desde", value=None)
    end = col_d2.date_input("Hasta", value=None)

    if start and end and start > end:
        st.error("Fecha 'Desde' mayor que 'Hasta'.")

    st.divider()
    st.header("2. Configuración Modelo")
    n_days = st.number_input("Días a predecir", min_value=1, max_value=90, value=7)
    test_size_days = st.number_input("Días validación", min_value=7, max_value=180, value=14)
    n_rolling_folds = st.number_input("Folds Backtest", min_value=1, max_value=10, value=1)
    
    st.divider()
    st.header("3. Optimización (Tuning)")
    do_tune = st.checkbox("Activar Tuning Automático", value=False, help="Desactiva para procesar más rápido.")
    n_param_samples = st.number_input("Intentos de Tuning", min_value=2, max_value=100, value=5, disabled=not do_tune)
    early_stopping = st.number_input("Rondas Early Stopping", min_value=0, max_value=300, value=10)

    st.divider()
    st.header("4. Escenarios")
    escenario_e = st.slider("Ajuste Entradas (%)", -50, 50, 0, 5)
    escenario_s = st.slider("Ajuste Salidas (%)", -50, 50, 0, 5)

    st.divider()
    btn_cargar = st.button("Cargar Datos de API", type="primary")
...
if btn_cargar:
    ...
    with st.spinner("Consultando FastAPI..."):
        try:
            ...
            if data:
                ...
                st.warning("La API devolvió 0 registros.")
...
if not rows:
    st.info("Configura los filtros y presiona 'Cargar Datos' para comenzar.")
    st.stop()
...
st.expander("Vista previa de datos cargados", expanded=False).dataframe(df.head())

# ==========================================
# 3. PROCESAMIENTO Y PREDICCIÓN (BUCLE PRINCIPAL)
# ==========================================
st.subheader("Generando Predicciones...")
...
for idx, ((g_edo, g_adm, g_suc), g0) in enumerate(groups):
    ...
    status_text.markdown(f"Procesando **{idx+1}/{total_groups}**: Sucursal `{g_suc}` (ADM: {g_adm})")
    ...
    if len(g0) < min_required:
        # Opcional: mostrar advertencia en consola o log
        continue
    ...
    # 3. Tuning de Hiperparámetros (Opcional)
    if do_tune:
        if g_adm not in param_cache:
            status_text.markdown(f"Optimizando modelo para ADM {g_adm}...")
    ...
    # Residuales para Bootstrap (regresando a escala original)
    ...
    for d in future_dates:
        ...
        xdf_row = xdf_row[FEATS] # Asegurar orden de columnas
...
if all_preds:
    ...
    # Métricas
    c1, c2 = st.columns(2)
    c1.metric("Sucursales Procesadas", len(all_preds))
    c2.metric("Total Días Predichos", len(all_preds_df))

    # Tabla Principal
    st.subheader("Tabla de Predicciones")
    cols_view = ["fecha_predicha", "prediccion", "limite_inferior", "limite_superior", "edo", "adm", "sucursal"]
    st.dataframe(all_preds_df[cols_view], use_container_width=True)
    ...
    with col_db:
        if st.button("Guardar en Base de Datos"):
            with st.spinner("Insertando registros en PostgreSQL..."):
                try:
                    save_predictions_to_db(all_preds_df)
                    st.balloons()
                    st.success(f"¡Éxito! Se guardaron {len(all_preds_df)} filas.")
                except Exception as e:
                    st.error(f"Error guardando en BD: {e}")

else:
    st.warning("⚠️ El proceso terminó sin generar predicciones.")
    st.markdown("Posibles causas: Filtros muy restrictivos o datos históricos insuficientes (se requieren mín. 60 días).")